...
def simple_tokenizer(text):
    """原生空格分词器，替代 torchtext.get_tokenizer"""
    return text.strip().split()


class SimpleVocab:
    """简单词表类，替代 torchtext.vocab"""

    def __init__(self, tokens, specials, default_index=0):
        self.token2idx = {tok: idx for idx, tok in enumerate(specials)}
        idx = len(specials)
        for tok in tokens:
    ...
    def __call__(self, tokens):
        return [self.token2idx.get(tok, self.default_index) for tok in tokens]
...
def build_vocab(token_lists, min_freq=1, specials=None):
    """从 token 列表构建词表，替代 build_vocab_from_iterator"""
    counter = Counter()
    for tokens in token_lists:
        counter.update(tokens)
    # 过滤低频词
    filtered_tokens = [tok for tok, freq in counter.items() if freq >= min_freq]
    return SimpleVocab(filtered_tokens, specials or [])
...
def construct_dataset_meta(train_iter):
    ...
    for ln in [SRC_LANG, TAR_LANG]:
        token_lists = list(yield_tokens(train_iter, tokenizer, ln))
        vocab_transform[ln] = build_vocab(
            token_lists, min_freq=1, specials=SPECIAL_TOKENS
        )
        vocab_transform[ln].set_default_index(UNK_IDX)
    src_vocab_size, tar_vocab_size = (
        len(vocab_transform[SRC_LANG]),
        len(vocab_transform[TAR_LANG]),
    )
...
if __name__ == "__main__":
    ...
    for split_dataset_fpath in split_dataset_fpaths:
        ...
        vocab_transform, text_transform, SRC_VOCAB_SIZE, TAR_VOCAB_SIZE = (
